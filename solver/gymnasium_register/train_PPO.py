...
model_path = Path("ppo_tic_tac_go.zip")
...
def save_graduation_checkpoint(model, model_path, grad_num, mean_reward, reward_std, mean_length):
    graduation_output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = graduation_output_dir / f"ppo_tic_tac_go_grad_{grad_num}.zip"
...
def learnProcess(num, threshold=None):
    ...
    env.unwrapped.terminate_on_repeated_states = False
    ...
    callbackEnv.unwrapped.terminate_on_repeated_states = False
